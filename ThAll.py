...
#for x in xrange(2,HW):
#	index = str(x)
#	xx = x - 1
#	index2 = str(xx)
#	dfOPT["o"+index] = np.where(dfOPT["o"+index] < dfOPT["o"+index2], dfOPT["o"+index2], dfOPT["o"+index])
...
swALL = pd.DataFrame()
for z in range(0, nrApp):
	swTh = pd.DataFrame({"sw":np.repeat(dfSW.ix[z],nrApp)})
	swALL = swALL.append(swTh, ignore_index=True)
print swALL
for y in weight:
	rDSS[y] = pd.DataFrame()
for x in xrange(1, HW):
	...
	rOPTALL["oa"+index] = dfOPTALL["oa"+index] / swALL["sw"]
	print rOPTALL
print rOPT
print rEASY
print rDSS
...
for x in xrange(1, HW):
	...
	optALL = pd.DataFrame()
	for z in range(0, nrApp):
		optTh = pd.DataFrame({"o"+index:np.repeat(dfOPT.ix[z,x-1],nrApp)})
		optALL = optALL.append(optTh, ignore_index=True)
	oOPTALL["oa"+index] = dfOPTALL["oa"+index] / optALL["o"+index]
print oSW
print oEASY
print oDSS
print oOPTALL
